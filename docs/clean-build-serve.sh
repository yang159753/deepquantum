#!/bin/bash
# 完整重建并启动服务：创建虚拟环境、清空所有缓存（耗时较长）
# 用法: ./clean-build-serve.sh [端口]
# 示例: ./clean-build-serve.sh       # 默认 8765
#       ./clean-build-serve.sh 8888
# 说明: 仅环境异常时使用。日常开发推荐 ./quick-build-serve.sh
...
echo ""
echo "==> 6. 启动服务 http://127.0.0.1:$PORT"
